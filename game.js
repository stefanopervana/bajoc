...
<p className="it">La mejor manera de aprender nuevas tecnologías es hacer algo interesante, divertida, atractiva y poco convencional. En este tutorial le guiará a través de un <a href="http://zofiakorcz.pl/mastermind">juego Mastermind</a> he construido para mostrar algunas de las ideas más fundamentales detrás de la biblioteca Reaccionar, especificación ECMAScript 6 y el módulo webpack bundler.</p>
...
<p className="it"><a href="http://www.ecma-international.org/ecma-262/6.0">ECMAScript 6</a> es la más reciente edición de la ECMAScript Language Specification, que estandariza el lenguaje de programación JavaScript. ES6 introduce una nueva sintaxis y características que vamos a echar un vistazo más de cerca más adelante en este artículo. Vale la pena mencionar que no todos los <a href="https://kangax.github.io/compat-table/es6/">navegadores son compatibles con</a> esas características de lujo por lo que es importante utilizar un compilador especial como <a href="https://babeljs.io/">Babel</a> para asegurarse de que nuestro código escrito en ES6 funcionará correctamente en los navegadores antiguos.</p>
...
<p className="it"><a href="https://facebook.github.io/react/">Reaccionar</a> es una biblioteca hecha por Facebook que utiliza un concepto de <code>Virtual DOM</code>, que tiene como objetivo aplicar el menor número de mutaciones como sea posible. Cuando cambia el estado del elemento, Reaccionar decide si una actualización DOM real es necesario mediante la comparación de los cambios en su <code>Virtual DOM</code>con el DOM real. La filosofía de reaccionar también se inclina hacia la separación limpia entre los componentes. Una de las características de la biblioteca es una <code>one-way data flow</code>que significa que los componentes hijos no pueden afectar directamente a los componentes principales.</p>
...
<p className="it">Antes de empezar quiero hacerle saber que todo el repositorio está disponible en <a href="https://github.com/Calanthe/mastermind">Calanthe / maestra</a> .</p>
...
<p className="it">La configuración entera webpack se especifica en el <code>webpack.config.js</code>archivo, que define las entradas, las salidas del proyecto y tipos de transformaciones que queremos llevar a cabo. Algunos de los fragmentos más importantes son:</p>
...
<p className="it">Si ejecuta el <code>npm run watch</code>especificado en el <code>package.json</code>archivo:</p>
...
<p className="it">Podrá cargar su archivo <code>///PATH_TO_FOLDER/dist/index.html</code> en el navegador.
...
<p className="it">Como se puede ver en la parte superior del archivo, no se importan tres módulos: <code>react</code>, <code>react-dom</code>que sirve como punto de entrada de los caminos de representación relacionados con DOM y nuestro código Mastermind. Después de la importación de módulos, el código Mastermind está inicializado y transmitidas en el <code>DOM</code>interior del elemento que <code>ID</code>está <code>mastermind</code>definido en el <code>./dist/index.html</code>archivo:</p>
...
<p className="it">En la <code>script</code>etiqueta se incluye el <code>game.js</code>archivo que debería estar en la misma <code>dist</code>carpeta que <code>index.html</code>. Si los puntos de entrada y salida del webpack están configurados correctamente, el <code>./dist/game.js</code>archivo debe ser compilado con base en lo anterior <code>./game.js</code>archivo de origen.</p>
...
<p className="it">El proyecto se configura así que vamos a centrarnos en la parte más interesante - la programación del juego Mastermind sí. El módulo de juego de conjunto se puede dividir en unos pocos componentes. Un componente es una clase Reaccionar, que debe ser responsable de una sola cosa. Como se mencionó antes, Reaccionar tiene <code>unidirectional data flow</code>por lo que es importante mantener la mayor cantidad de componentes como sea posible <code>stateless</code>. Como desarrolladores de Facebook se recomiendan:</p>
...
<blockquote>
<p className="it">Un patrón común es crear varios componentes sin estado que acaba de representar datos, y tienen un componente de estado por encima de ellos en la jerarquía que pasa a su estado a sus niños a través de los apoyos. El componente con estado encapsula toda la lógica de la interacción, mientras que los componentes sin estado cuidan de representación de datos de una manera declarativa.</p>
...
<p className="it">jerarquía de los componentes integrales es más fácil de leer en el siguiente diagrama:</p>
...
<p className="it">Esto es como estos componentes se ven como en extensión Reaccionar del cromo:</p>
...
<p className="it">Una de las primeras novedades de ES6 quiero introducir a usted <code>modules</code>. Si se han fijado anteriormente en el <code>./game.js</code>archivo, Importé tres módulos en la parte superior de ese archivo:</p>
...
<p className="it">Los módulos son ya conocidos por la comunidad front-end. No se ejecutaron a través de diferentes bibliotecas como <a href="http://requirejs.org">RequireJS</a> o <a href="http://browserify.org/">Browserify</a> pero afortunadamente ahora podemos utilizar la solución nativa construida en ES6. Uno de los principales de los módulos es que tienen que especificarse en un archivo dedicado. Cada módulo es un conjunto unitario, lo que significa que existe a lo sumo una instancia del módulo independientemente del número de veces que se importó. El módulo completo se ejecuta una vez que se carga y su contenido se encapsula, la prevención de la contaminación del espacio de nombres global.</p>
...
<p className="it">Exportación de módulos en ES6 es muy fácil y sencillo:</p>
...
<p className="it">Tenemos unos cuantos Reaccionar clases definidas en el <code>./src/mastermind</code>archivo, pero queremos exponer un solo valor. La <code>default export</code>sintaxis significa que los otros módulos ES6 sólo pueden acceder a que una clase en particular, y sólo puede ser leído sin ninguna modificación. El <code>./src/mastermind</code>módulo exporta la <code>Mastermind</code>clase porque se trata de un módulo de niño y no puede exportar módulos que se han definido en otro lugar.</p>
...
<p className="it">Vamos a echar un vistazo más profundo a la <code>Mastermind</code>clase misma. Como se mencionó antes, es la principal clase que encapsula Reaccionar subclases y pasar los datos a ellos. En Reaccionar hay dos tipos de propiedades: <code>props</code>y <code>states</code>y la diferencia entre ellos es crucial para entender toda la filosofía detrás de React. Ambos son objetos JS lisos y sus cambios disparan <code>render()</code>actualización. <code>Props</code>se pasan al niño dentro del método de procesamiento de los padres y son inmutables en los componentes hijos. Los componentes hijos deben ser lo más <code>stateless</code>posible y simplemente hacer que esos <code>props</code>valores. El mejor ejemplo de tales <code>stateless</code>componentes en el juego Mastermind es el <code>SubmitButton</code>:</p>
...
<p className="it">Sobre la base de <code>props</code>pasar por su módulo padre, que hace que un <code>&lt;button&gt;</code>elemento con atributos propios como nombre de clase y <code>onClick</code>método. Como se puede ver, esos <code>props</code>valores son accesibles aquí a través de <code>this.props</code>objeto.</p>
...
<p className="it">Pero ¿cómo podemos cambiar esos valores pasados? Esto es lo que <code>states</code>son para. Al contrario de los componentes hijos, los padres son los componentes <code>stateful</code>, lo que significa que por lo general basado en la acción del usuario, pueden mutar los <code>states</code>valores y pasarlos hacia abajo. Por eso <code>props</code>y <code>state</code>están relacionados. El <code>state</code>de un componente a menudo convertido en el <code>props</code>de un componente secundario. Como aquí:</p>
...
<p className="it">El <code>codeLength</code>y <code>colors</code>se pasan al <code>Mastermind</code>módulo y se accede allí por <code>this.props.colors</code>y <code>this.props.codeLength</code>valores:</p>
...
<p className="it">El <code>selectedPeg</code> <code>state</code>es inicializado en el predefinida <code>getInitialState()</code>función que se ejecuta exactamente una vez durante el ciclo de vida del componente y establece todo el estado:</p>
...
<p className="it">A pesar de la <code>getInitialState</code>método, el <code>Mastermind</code>módulo también contiene otras funciones:</p>
...
<p className="it">Esto se considera que es una buena práctica, ya que el <code>stateful</code> <code>Mastermind</code>componente tiene todas las funciones de devolución de llamada y pasar al estado a través <code>props</code>. Cuando las funciones de devolución de llamada llaman <code>this.setState()</code>método y mutan <code>states</code>, Reaccionar se encarga de componentes re-representación. Como podrá darse cuenta, cada uno de los componentes tienen un <code>render</code>método. Es un método requerido responsable sólo de devolver el elemento hijo único. Como se puede ver siempre devuelve cualquiera de <code>HTML</code>las etiquetas u otro componente secundario que finalmente pagará a HTML.</p>
...
<p className="it">Como se mencionó antes, he utilizado la <code>JSX</code>extensión de la sintaxis. Es posible saltar <code>JSX</code>y utilizar Javascript llanura. En lugar de:</p>
...
<p className="it">Podría escribir:</p>


<p className="p">I could write:</p>

<div className="image"><a></a><img src="/static/g016.jpg" width="100%" alt="Image"/></div>
<p className="it">Yo prefiero usar la <code>JSX</code>extensión, porque es más familiar sintaxis para la definición de las estructuras de árbol con los atributos.</p>
...
<p className="it">Creo que he cubierto todos los elementos básicos más importantes de React biblioteca. Será más fácil ahora para entender cómo funciona el juego Mastermind.</p>
...
<p className="it">Asumo que ya está familiarizado con las reglas del juego Mastermind. Si no es así, echar un vistazo a esto útil <a href="https://en.wikipedia.org/wiki/Mastermind_(board_game)">descripción de Wikipedia</a> .</p>
...
<p className="it">Lo primero que hay que hacer al principio del juego es crear el código de auto que el usuario tendrá que adivinar genera. Lo hacemos en el <code>getCode</code>método:</p>
...
<p className="it">El código anterior es sencillo una vez que comprendes el nuevo ES6 <code>arrow functions</code>. La expresión <code>generateCode = (i) =&gt; &#123;i&#125;;</code>es una sintaxis más corta que <code>generateCode = function (i) &#123; return i &#125;;</code>. <br></br>No sólo se ve mucho mejor, pero también se une léxico <code>this</code>, así que no hay necesidad de utilizar <code>.bind()</code><br></br>o <code>var that = this</code>, más.</p>
...
<br></br>It not only looks much better but it also lexically binds <code>this</code>, so there is no need to use <code>.bind()</code><br></br> or <code>var that = this</code>, anymore.</p>

<p className="it">También he empezado a utilizar las palabras clave <code>const</code>y <code>let</code>para declarar una variable. Uno de los mayores inconvenientes de ES5 es que <code>var</code>crea una variable con ámbito dentro de su función principal más próximo. Esto conduce a la <a href="http://ignaciothayer.com/post/a-dangerous-example-of-javascript-hoisting/">elevación de los problemas</a> que a veces requiere el uso <code>closures</code>de arreglar. <code>Let</code>alcances la variable al bloque más cercano, que incluye también los bucles y sentencias condicionales. La principal diferencia entre <code>let</code>y <code>const</code>es que la <code>const</code>declaración crea un solo lectura <code>reference</code>a un valor lo que una vez definido no puede ser reasignada.</p>
...
<p className="it">El <code>times</code>método es un <a href="http://stackoverflow.com/a/34175903">funcional-ish</a> método creado para impedir que me utilizando <code>for var</code>para iterar n veces. He encontrado que la solución muy limpia y útil.</p>
...
<p className="it">La junta de decodificación situado a la izquierda consta de diez filas, donde cada uno de ellos incluye:</p>


<p className="p">The decoding board located on the left consists of ten rows, where each of them includes:</p>

<ul>
<li className="it"><code>DecodeRow</code>, Donde el usuario hace que él / ella intentos,</li>
...
<li className="it"><code>HintsRow</code> para indicar qué clavijas son elegidos correctamente.</li>

<li className="p"><code>HintsRow</code> to indicate which pegs are chosen correctly.</li>
</ul>

<p className="it">El <code>key</code>valor que se pasa abajo a cada componente secundario es un valor necesario que ayuda Reaccionar manejar <code>DOM</code>cambios de una manera mínima.</p>
...
<p className="it">El <a href="https://github.com/JedWatson/classnames">classnames</a> módulo fue originalmente parte de reaccionar, pero ahora se pone de pie como una biblioteca de utilidad adicional. Es un módulo muy útil que ayuda a unirse <code>classNames</code>juntos. En lugar de:</p>
...
<p className="it">Algunas cosas interesantes que está sucediendo en la <code>DecodeRow</code>clase. En primer lugar, yo no quería para actualizar todas las filas ya probada. En tales situaciones, el <code>shouldComponentUpdate</code>método es muy útil:</p>
...
<p className="it">El <a href="https://facebook.github.io/react/docs/advanced-performance.html#avoiding-reconciling-the-dom">shouldComponentUpdate</a> es muy eficaz en situaciones en las que estamos seguros de que la re-representación del componente es redundante. La <code>nextProps</code>variable tiene acceso a las propiedades pasados a este componente y sobre esa base se puede calcular si hay una necesidad de proceder a re-representación.</p>
...
<p className="it">El <code>render</code>método de la <code>DecodeRow</code>componente es el siguiente:</p>
...
<p className="it">Clase CSS de la clavija depende de las clavijas preseleccionados situados en el lado derecho, lo que me acaba de llamar <code>CodePegs</code>. Este es el lugar donde el usuario puede seleccionar los colores que él / ella desea utilizar en la tabla de decodificación de la izquierda. Esos valores seleccionados se almacenan en una <code>Map</code>llamada <code>currentGuess</code>y pueden tomar cualquier valor predefinido en los <code>./game.js</code>valores: cero, uno, dos, tres, cuatro, cinco. Es importante en este punto para pasar el valor correcto por lo que los estilos se aplicarán de acuerdo a las clavijas elegidos desde la derecha.</p>
...
<p className="it">El <code>Peg</code>componente se encarga de mostrar marcado basado en el pasado <code>props</code>. Este es otro ejemplo de un <code>stateless</code>componente:</p>
...
<p className="it">Cada clavija en el juego (en ambos lados izquierdo y derecho) se representa como <code>&lt;input type='radio'/&gt;</code>el que se encarga de almacenar los valores necesarios y correspondiente <code>&lt;label&gt;</code>elemento que se utiliza sólo con fines de estilo. Así es como se labran clavijas:</p>
...
<p className="it">Como probablemente ya descubierto, el <code>className</code>valor establece el atributo de clase css. Se llama <code>className</code>en lugar de simplemente <code>class</code>, porque <code>JSX</code>se traduce a JS, donde <code>class</code>ya está en uso.</p>
...
<p className="it">Clavijas en el tablero de decodificación situado a la izquierda cambian su estado (que se representa como colores) sobre la base de la paridad seleccionada a la derecha. Por defecto se selecciona la primera clavija (una naranja), que está marcado con un borde más oscuro. Como se mencionó antes, el <code>Peg</code>es el módulo más bajo en la jerarquía. Las acciones del usuario son manejadas por el <code>onClick</code>atributo <code>onClick=&#123;this.props.activatePeg&#125;</code>.</p>
...
<p className="it">El <code>activatePeg</code>método se define en el <code>Mastermind</code>módulo:</p>
...
<p className="it">El nuevo <code>selectedPeg</code> <code>state</code>se establece cuando se selecciona una de las clavijas correctas. Cada clavija en el juego se representa como un <code>&lt;input&gt;</code>por lo que su valor es muy fácil de conseguir. Después de cambiar el <code>selectedPeg</code>estado, Reaccionar se encarga de la re-representación y la clavija seleccionado se configura como activo.</p>
...
<p className="it">Se puede ver aquí el <code>startsWith</code>método ES6, que determina si una cadena 'clavija' comienza con los caracteres de otra cadena, en este ejemplo - nombre de la entrada en el <code>CodePegs</code>tablero.</p>
...
<p className="it">En la situación en la que se selecciona la paridad en el tablero de decodificación, el <code>currentGuess</code>estado se actualiza: <code>this.setState(&#123; currentGuess: this.state.currentGuess.set(event.target.value - 1, this.state.selectedPeg) &#125;);</code>. El <code>currentGuess</code>es una <code>Map</code>estructura, donde un valor adecuado identificado por una <code>event.target.value - 1</code>clave, tiene que ser cambiado a la clavija preseleccionada: <code>this.state.selectedPeg</code>.</p>
...
<p className="it">Una vez más, después de cambiar el <code>state</code>Reaccionar hace que la placa de actualización.</p>
...
<p className="it">Después de la elección de cuatro clavijas, el usuario puede enviar una conjetura para comprobar si son correctos. El botón de envío es visible sólo cuando todas las cuatro clavijas en una fila se seleccionan. En la selección, la <code>this.props.submitPegs</code>que se llama:</p>
...
<p className="it">Primero necesitamos para copiar el <code>Map</code>objeto que representa un código de color generado aleatoriamente (que - como un recordatorio - consta de 4 clavijas). Es importante hacer un clon del objeto, porque más adelante vamos a eliminar algunos de sus valores y no queremos alterar el código de color principal. Después de inicializar los valores, hacemos dos pasos importantes. En primer lugar, queremos ir a través de las clavijas seleccionados y comparar sus valores y posiciones con el código de color generado. Si hay algún clavijas con la misma posición y el color, tenemos que sacarlos de <code>Maps</code>porque no los queremos en el siguiente paso de cálculos. También tenemos que contar esas clavijas cuya posición y valores son los mismos, con el fin de mostrar los círculos pistas sobre valores coincidentes (de estilo como clavijas negras). Aviso, que a fin de bucle a través de <code>Map</code>objetos, tenemos que utilizar la <code>for (let [key, value] of pegs) &#123;&#125;</code>sintaxis.</p>
...
<p className="it">En el segundo paso, tenemos que encontrar todas las clavijas seleccionados que están en cualquier parte del código y la marca más tarde con un color blanco. Para ello he utilizado la implementación simple de la <code>keyOf</code>función, que devuelve bien <code>-1</code>o la tecla del elemento encontrado.</p>
...
<p className="it">Tengo que admitir que el algoritmo anterior no fue inventada por mí. He encontrado esta <a href="http://codereview.stackexchange.com/questions/27710/how-can-i-improve-this-version-of-the-board-game-mastermind">discusión muy interesante</a> y la aplicación de dichos hallazgos a mi juego.</p>
...
<p className="it">Después de la validación de las clavijas, tenemos que incrementar el <code>currentRow</code>valor, restablezca el <code>currentGuess</code> <code>Map</code>, por lo que el usuario puede tomar otra conjetura y actualizar el <code>exactMatches</code>y <code>valueMatches</code>que necesita el <code>HintsRow</code>componente.</p>
...
<p className="it"><code>HintsRow</code>proporciona retroalimentación acerca de clavijas seleccionadas. La pequeña clavija negro se coloca para cada clavija de código que es correcta en color y posición. Así que debe haber <code>exactMatches</code>clavijas negras y <code>valueMatches</code>clavijas de sugerencia blancos:</p>
...
<p className="it">Si todas las clavijas seleccionados por el usuario están en la ubicación correcta, tenemos que actualizar el <code>endGame</code>estado e informar al usuario sobre el éxito. Si la fila actual es el último y el usuario todavía no adivinar el patrón, entonces el juego ha terminado.</p>
...
<p className="it">Sé que esto es más bien un post largo y realmente aprecio que usted se quedó conmigo hasta aquí :). Me he centrado sólo en las partes más importantes del juego. El código relacionado con una mayor funcionalidad como volver a cargar el juego y alternar las reglas del juego que dejará como ejercicio para usted!</p>
...
<p className="it">Vamos a resumir lo que hemos aprendido acerca de ECMAScript 6 y reaccionar biblioteca.</p>
...
<ul>
<li className="it">Hay nuevas <code>let</code>y <code>const</code>las declaraciones de variables, sobre todo para evitar la elevación.</li>

<li className="p">There are new <code>let</code> and <code>const</code> variable declarations, mostly to avoid hoisting.</li>
<li className="it">El <code>str.startsWith()</code>es uno de los más nuevo método de cadena, lo que determina si una cadena comienza con los caracteres de otra cadena.</li>
...
<li className="it">Utilice <code>for (var value of myArray) &#123;&#125;</code>para recorrer <code>Maps</code>.</li>

<li className="p">Use <code>for (var value of myArray) &#123;&#125;</code> to iterate through <code>Maps</code>.</li>
<li className="it">Funciones de dirección <code>=&gt;</code>son útiles cuando se desea enlazar automáticamente <code>this</code>.</li>

<li className="p">Arrow functions <code>=&gt;</code> are useful when you want to automatically bind <code>this</code>.</li>
<li className="it">Es posible crear y módulos de importación.</li>
...
<ul>
<li className="it">Un componente es una clase Reaccionar, que idealmente es responsable de una sola cosa.</li>

<li className="p">A component is a React class, which ideally is responsible for one thing only.</li>
<li className="it">Siempre devuelva un elemento DOM que envuelve los componentes hijos o marcado en el interior.</li>
...
<li className="it">La <code>className</code>palabra clave se utiliza porque <code>class</code>se toma en ES6. Así <code>htmlFor</code>en lugar de <code>for</code>.</li>
...
<li className="it"><code>Props</code> son inmutables en los componentes hijos.</li>

<li className="p"><code>Props</code> are immutable in the child components.</li>
<li className="it"><code>States</code>son mutables. Después de alterarlos, Reaccionar se encarga de componentes re-representación.</li>
...
<li className="it">Componentes lo menos posible volver a hacer, recordar sobre el <code>shouldComponentUpdate</code>método.</li>
...
<p className="it">En este ejemplo particular de reacción puede no ser más rápido que las operaciones DOM nativas estándar. Hay <a href="https://objectpartners.com/2015/11/19/comparing-react-js-performance-vs-native-dom/">varias </a> <a href="https://news.ycombinator.com/item?id=9638305">discusiones</a> sobre el rendimiento de esa biblioteca. A pesar de que se lo recomendaría a utilizar incluso para los pequeños juegos como este. Al menos que usted consiga conocer la tecnología más reciente es mejor en una manera divertida, interesante.</p>
...
<p className="it">Muchas gracias a <a href="https://twitter.com/robjamesberry">Rob Berry</a> y <a href="https://twitter.com/sherbet_noin">Richard Nguyen</a> para guiarme a mí al escribir este juego y el artículo.</p>
